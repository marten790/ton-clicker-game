...
import { Enemy } from '../../types/global';
...
const EnemyArea = ({ enemy, onAttack }: EnemyAreaProps) => {
  ...
  useEffect(() => {
    ...
    setEnemyImage(`/api/placeholder/${placeholderSize}/${placeholderSize}?text=Mushroom+${imageIndex}`);
  }, [enemy.id]);
  ...
  return (
    <div className="relative bg-gray-800 p-4 rounded-lg shadow-inner mb-4">
      <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-cover bg-center rounded-lg" 
           style={{ backgroundImage: 'url(/api/placeholder/600/400?text=Forest+Background)' }} />
